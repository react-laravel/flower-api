 ...
 * Composes IdempotencyLocking and IdempotencyCaching traits for separation of concerns.
 *
...
trait Idempotency
{
    use IdempotencyLocking, IdempotencyCaching;

    /**
     * Initialize the idempotency service using Laravel's service container.
     * This fixes DIP violation by depending on the container rather than
     * directly instantiating the concrete class.
     */
    ...
    protected function handleIdempotentRequest(
        ...
        return $this->processWithIdempotencyKey($idempotencyKey, $handler);
    }

    /**
     * Process a request with an idempotency key.
     * Handles lock acquisition, pending state check, and response caching.
     */
    private function processWithIdempotencyKey(string $idempotencyKey, callable $handler): JsonResponse
    {
    ...
    /**
     * Handle the case when lock could not be acquired.
     */
    private function handleLockNotAcquired(string $idempotencyKey): JsonResponse
    {
    ...
    /**
     * Execute the handler with idempotency key handling.
     */
    private function executeHandlerWithIdempotency(string $idempotencyKey, callable $handler): JsonResponse
    {
